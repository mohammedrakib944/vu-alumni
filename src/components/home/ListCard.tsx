...
const ListCard = ({ user }: any) => {
  return (
    <Link href={`/user/${user?._id}`}>
      <div className="bg-gray-50 rounded-lg overflow-hidden border  border-black/20 flex items-center gap-3 hover:shadow-xl duration-150">
        <img
          src={
            user?.photo ||
            `https://api.dicebear.com/7.x/notionists/svg?seed=${user?._id}`
          }
          className="w-[150px] md:w-[230px] h-[130px] md:h-[160px] object-cover bg-gradient-to-tr from-primary to-green-400"
          alt="Rakib"
        />
